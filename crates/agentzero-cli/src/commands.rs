...
async fn cmd_chat(
    ...
    // Priority: AZ_PASSPHRASE env var > machine-derived key (no prompt needed).
    let passphrase = if encrypt {
        let pass = if let Ok(env_pass) = std::env::var("AZ_PASSPHRASE") {
            ...
            // Derive from machine identity — no prompt needed
            agentzero::core::crypto::machine_passphrase()
        };
        Some(pass)
    } else {
        None
