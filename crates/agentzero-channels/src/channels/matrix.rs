                    ...
                    for (room_id, room_data) in rooms {
                        if let Some(events) = room_data["timeline"]["events"].as_array() {
                            for event in events {
                                ...
                                let msg = ChannelMessage { id: helpers::new_message_id(), sender: sender.to_string(), reply_target: room_id.to_string(), content: body.to_string(), channel: "matrix".to_string(), timestamp: helpers::now_epoch_secs(), thread_ts: None };
