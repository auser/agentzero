//! Codex CLI delegation tool — invokes the `codex` CLI as a subprocess
//! for two-tier agent delegation.
//!
//! AgentZero can delegate complex coding tasks to Codex CLI, which runs
...
/// Configuration for the Codex CLI delegation tool.
#[derive(Debug, Clone)]
pub struct CodexCliConfig {
    /// Maximum time to wait for the `codex` process (default: 300s).
    pub timeout: Duration,
    /// Maximum output bytes to capture (default: 64 KiB).
    pub max_output_bytes: usize,
    /// If set, constrains `codex` to this working directory.
...
struct Input {
    /// The task/prompt to send to Codex CLI.
    task: String,
...
/// Tool that delegates tasks to the `codex` CLI (OpenAI's Codex CLI).
///
/// Runs `codex --quiet "{task}"` as a subprocess, captures output,
/// and returns it to the calling agent.
#[tool(
    name = "codex_cli",
    description = "Delegate a task to Codex CLI (OpenAI's coding agent). Runs as an independent agent with filesystem and shell access."
)]
...
#[allow(dead_code)]
struct CodexCliSchema {
    /// The task or prompt to delegate to Codex CLI
    task: String,
...
impl Default for CodexCliTool {
    fn default() -> Self {
        Self::new(CodexCliConfig::default())
    }
}
...
impl Tool for CodexCliTool {
    ...
    fn input_schema(&self) -> Option<serde_json::Value> {
        Some(CodexCliSchema::schema())
    }

    async fn execute(&self, input: &str, ctx: &ToolContext) -> anyhow::Result<ToolResult> {
        let parsed: Input = serde_json::from_str(input)
            .map_err(|e| anyhow::anyhow!("codex_cli expects JSON: {{\"task\": \"...\"}}: {e}"))?;
        ...
        let codex_path = which_codex().await?;
        ...
        match tokio::time::timeout(timeout_duration, child.wait_with_output()).await {
            ...
            Ok(Err(e)) => Err(anyhow::anyhow!("codex CLI execution failed: {e}")),
            Err(_) => {
...
async fn which_codex() -> anyhow::Result<String> {
    for name in &["codex"] {
...
mod tests {
    ...
    async fn invalid_json_returns_error() {
        ...
        assert!(err.to_string().contains("codex_cli expects JSON"));
    }
