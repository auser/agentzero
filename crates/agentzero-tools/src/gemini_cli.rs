//! Gemini CLI delegation tool — invokes the `gemini` CLI as a subprocess
//! for two-tier agent delegation.
//!
//! AgentZero can delegate complex coding tasks to Gemini CLI, which runs
...
/// Configuration for the Gemini CLI delegation tool.
#[derive(Debug, Clone)]
pub struct GeminiCliConfig {
    /// Maximum time to wait for the `gemini` process (default: 300s).
    pub timeout: Duration,
    /// Maximum output bytes to capture (default: 64 KiB).
    pub max_output_bytes: usize,
    /// If set, constrains `gemini` to this working directory.
...
struct Input {
    /// The task/prompt to send to Gemini CLI.
    task: String,
...
#[allow(dead_code)]
struct GeminiCliSchema {
    /// The task or prompt to delegate to Gemini CLI
    task: String,
...
/// Tool that delegates tasks to the `gemini` CLI (Google's Gemini CLI).
///
/// Runs `gemini -p "{task}"` as a subprocess, captures output,
/// and returns it to the calling agent.
#[tool(
    name = "gemini_cli",
    description = "Delegate a task to Gemini CLI (Google's coding agent). Runs as an independent agent with filesystem and shell access."
)]
...
impl Default for GeminiCliTool {
    fn default() -> Self {
        Self::new(GeminiCliConfig::default())
    }
}
...
impl Tool for GeminiCliTool {
    ...
    fn input_schema(&self) -> Option<serde_json::Value> {
        Some(GeminiCliSchema::schema())
    }

    async fn execute(&self, input: &str, ctx: &ToolContext) -> anyhow::Result<ToolResult> {
        let parsed: Input = serde_json::from_str(input)
            .map_err(|e| anyhow::anyhow!("gemini_cli expects JSON: {{\"task\": \"...\"}}: {e}"))?;
        ...
        let gemini_path = which_gemini().await?;
        ...
        match tokio::time::timeout(timeout_duration, child.wait_with_output()).await {
            ...
            Ok(Err(e)) => Err(anyhow::anyhow!("gemini CLI execution failed: {e}")),
            Err(_) => {
...
/// Find the `gemini` binary in PATH.
async fn which_gemini() -> anyhow::Result<String> {
    for name in &["gemini"] {
    ...
    Err(anyhow::anyhow!(
        "gemini CLI not found in PATH. Install it from https://github.com/google-gemini/gemini-cli"
    ))
}
...
mod tests {
    ...
    async fn input_schema_is_valid_json() {
        let tool = GeminiCliTool::default();
    ...
    async fn invalid_json_returns_error() {
        ...
        assert!(err.to_string().contains("gemini_cli expects JSON"));
    }
