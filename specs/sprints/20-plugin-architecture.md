...
- [x] Implement `plugin update [<id>]` (check registry for newer versions) — completed in Phase 8
- [x] Implement `plugin search <query>` (search registry index) — completed in Phase 8
- [x] Implement `plugin outdated` (list plugins with updates available) — completed in Phase 8
...
- [x] `cargo build -p agentzero --release` compiles with full plugin support
- [x] `cargo build -p agentzero --profile release-min --no-default-features --features minimal` stays ~5MB
- [x] `cargo test --workspace` — 1435 tests pass, 0 failures across 98 test suites
- [x] Sample plugin: build → package → install → discover → execute → verify output (SDK integration tests)
- [x] CWD hot-reload: drop `.wasm` → tool reloads without restart (watcher unit tests)
- [x] `plugin disable` → not loaded next startup (filter_by_state tests)
- [x] `plugin search` → finds registry results (registry search tests)
- [x] FFI tool registration works (7 FFI tests: register, execute, error propagation)
- [x] Plugin timeout/memory limits enforced (v1 + v2 timeout tests, memory limit tests)
- [x] Plugin failure returns error, never crashes agent (invalid JSON, missing entrypoint, unknown board, duplicate ID, missing action tests)
- [x] Binary size budgets unchanged (default: 18MB < 30MB, minimal: 5.0MB < 6MB)

### Current Measurements (Sprint 20 — Post Plugin Architecture)

| Metric                     | Default (release) | Minimal (release-min) |
| -------------------------- | ----------------- | --------------------- |
| Binary size (macOS arm64)  | 18 MB             | 5.0 MB                |
| Workspace tests            | 1435 passing      | —                     |
| Test suites                | 98                | —                     |
