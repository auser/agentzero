...
Both `OllamaProvider` and `OpenAICompatProvider` implement this trait. Adding a new provider means implementing `ModelProvider` and registering a `ProviderType` variant.
