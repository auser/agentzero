...
WASM execution is denied by default. To run WASM skills, explicitly set this to `allow` or `require_approval`. The binary must also be built with `--features wasm`.
