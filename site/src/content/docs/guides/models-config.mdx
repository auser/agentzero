...
| `type` | string | `ollama` or `openai-compatible` |
...
Uses the OpenAI `/v1/chat/completions` endpoint. Works with llama.cpp, vLLM, LM Studio, LocalAI, and any other server that implements the OpenAI chat completions API.
...
```bash
az chat --local --provider lm-studio --model gemma-4-12b
```
