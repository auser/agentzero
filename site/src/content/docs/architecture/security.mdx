...
|------|----------|-----------|
| None | Instruction-only skills | No code executed |
| Host Readonly | Safe read-only tools | Filesystem access, path-restricted |
| Host Supervised | Tools with write/shell | Policy-gated, audited |
| **WASM Sandbox** | Portable tools | No filesystem, no network, fuel-limited |
| MVM MicroVM | High-risk execution | Full VM isolation (planned) |
| Deny | Blocked | Nothing executes |
