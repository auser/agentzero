...
### `agentzero-skills`
Skill manifests, security scanner, report generator, skill registry with lockfile. Includes `load_manifest()` for building `SkillManifest` from SKILL.md frontmatter and `find_wasm_module()` for locating `.wasm` files.
