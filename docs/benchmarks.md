...
Run core-loop harness (offline, no external provider call):
...
| YYYY-MM-DD | `<sha>` | macOS/Linux + CPU | core_loop_single_turn | 500 |  |  |  | from bench output |
| YYYY-MM-DD | `<sha>` | macOS/Linux + CPU | core_loop_tool_turn | 500 |  |  |  | from bench output |
